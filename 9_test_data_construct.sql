--wifi to row
drop table if exists online_test_wifi_in_row;
create table online_test_wifi_in_row 
...
jar -libjars wifi.jar -classpath ./wifi.jar wifi.WifiToRow evaluation_public_wifi_screen online_test_wifi_in_row;

--构造训练架子
drop table if exists online_test_struct;
create table online_test_struct as
...
d.w_c
from
	(select a.row_id,a.mall_id,b.shop_id from evaluation_public a left outer join shop_info b on a.mall_id=b.mall_id) t
left outer join online_test_wifi_in_row d on t.row_id=d.row_id;

--往架子上加强度特征
drop table if exists online_test_struct_2;
create table online_test_struct_2 as select
						...
						from
							...
							from
								...
								from
									...
									from online_test_struct a
									left outer join online_test_wifi_range w on a.w_1=w.wifi_id and a.shop_id=w.shop_id) b
								left outer join online_test_wifi_range w on b.w_2=w.wifi_id and b.shop_id=w.shop_id) c
							left outer join online_test_wifi_range w on c.w_3=w.wifi_id and c.shop_id=w.shop_id) d
						left outer join online_test_wifi_range w on d.w_4=w.wifi_id and d.shop_id=w.shop_id) e
					left outer join online_test_wifi_range w on e.w_5=w.wifi_id and e.shop_id=w.shop_id) f
				left outer join online_test_wifi_range w on f.w_6=w.wifi_id and f.shop_id=w.shop_id) g
			left outer join online_test_wifi_range w on g.w_7=w.wifi_id and g.shop_id=w.shop_id) h
		left outer join online_test_wifi_range w on h.w_8=w.wifi_id and h.shop_id=w.shop_id) i
	left outer join online_test_wifi_range w on i.w_9=w.wifi_id and i.shop_id=w.shop_id) j
left outer join online_test_wifi_range w on j.w_10=w.wifi_id and j.shop_id=w.shop_id;
--往架子上加wifi-shop-count
drop table if exists online_test_struct_3;
create table online_test_struct_3 as select
k.*,t.wifi_shop_count
from
	(select j.*,w.wsc_10
	from
						...
						from
							...
							from
								...
								from
									...
									from
										(select a.*,w.wsc_1
										from online_test_struct_2 a
										left outer join online_test_wifi_shop_count w on a.w_1=w.wifi_id and a.shop_id=w.shop_id) b
									left outer join online_test_wifi_shop_count w on b.w_2=w.wifi_id and b.shop_id=w.shop_id) c
								left outer join online_test_wifi_shop_count w on c.w_3=w.wifi_id and c.shop_id=w.shop_id) d
							left outer join online_test_wifi_shop_count w on d.w_4=w.wifi_id and d.shop_id=w.shop_id) e
						left outer join online_test_wifi_shop_count w on e.w_5=w.wifi_id and e.shop_id=w.shop_id) f
					left outer join online_test_wifi_shop_count w on f.w_6=w.wifi_id and f.shop_id=w.shop_id) g
				left outer join online_test_wifi_shop_count w on g.w_7=w.wifi_id and g.shop_id=w.shop_id) h
			left outer join online_test_wifi_shop_count w on h.w_8=w.wifi_id and h.shop_id=w.shop_id) i
		left outer join online_test_wifi_shop_count w on i.w_9=w.wifi_id and i.shop_id=w.shop_id) j
	left outer join online_test_wifi_shop_count w on j.w_10=w.wifi_id and j.shop_id=w.shop_id) k
left outer join online_test_wifi_shop_count_all_position t on k.w_1=t.wifi_id and k.shop_id=t.shop_id;
--往架子上加wifi-count
drop table if exists online_test_struct_4;
create table online_test_struct_4 as select
k.*,t.wifi_count
from
	(select j.*,w.wc_10
	from
						...
						from
							...
							from
								...
								from
									...
									from
										(select a.*,w.wc_1
										from online_test_struct_3 a
										left outer join online_test_wifi_count w on a.w_1=w.wifi_id) b
									left outer join online_test_wifi_count w on b.w_2=w.wifi_id) c
								left outer join online_test_wifi_count w on c.w_3=w.wifi_id) d
							left outer join online_test_wifi_count w on d.w_4=w.wifi_id) e
						left outer join online_test_wifi_count w on e.w_5=w.wifi_id) f
					left outer join online_test_wifi_count w on f.w_6=w.wifi_id) g
				left outer join online_test_wifi_count w on g.w_7=w.wifi_id) h
			left outer join online_test_wifi_count w on h.w_8=w.wifi_id) i
		left outer join online_test_wifi_count w on i.w_9=w.wifi_id) j
	left outer join online_test_wifi_count w on j.w_10=w.wifi_id) k
left outer join online_test_wifi_count_all_position t on k.w_1=t.wifi_id;
--并入杂特征
drop table if exists online_test_struct_5;
create table online_test_struct_5 as select
			...
			from
				...
				from
					...
					from
						...
						from 
							(select a.*,b.user_id,regexp_count(b.wifi_infos,'null') as contain_null,b.longitude,b.latitude
							from online_test_struct_4 a left outer join evaluation_public b on a.row_id=b.row_id) c
						left outer join online_test_shop_freq d on c.shop_id=d.shop_id) e
					left outer join online_test_shop_center f on e.shop_id=f.shop_id) g
				left outer join shop_info h on g.shop_id=h.shop_id) i
			left outer join online_test_user_shop j on i.user_id=j.user_id and i.shop_id=j.shop_id) k
		left outer join online_test_user_mall l on k.user_id=l.user_id and k.mall_id=l.mall_id) m
	left outer join online_test_connect_count n on m.w_c=n.wifi_id) o
left outer join online_test_connect_shop_count p on o.w_c=p.wifi_id and o.shop_id=p.shop_id;
--并入shop_block特征
drop table if exists online_test_struct_6;
create table online_test_struct_6 as select 
...
from
	...
	left outer join online_test_block5_shop_count j
...
left outer join online_test_block10_shop_count l
...
--提特征
drop table if exists online_test_feature;
create table online_test_feature as select
t.*,
...
from
	...
	from online_test_struct_6) t;

drop table if exists online_test_feature_with_tfidf;
create table online_test_feature_with_tfidf as select
c.*,
...
from
	...
	from online_test_feature a left outer join online_test_tfidf b on a.row_id=b.row_id and a.shop_id=b.shop_id) c
left outer join online_test_gpsadc_one d on c.row_id=d.row_id and c.shop_id=d.shop_id;

drop table if exists online_test_feature_with_multi;
create table online_test_feature_with_multi as 
	select a.*,if(b.prob is null,0,b.prob) as prob_multi from online_test_feature_with_tfidf a
	left outer join online_test_multi_prob_normal b on a.row_id=b.row_id and a.shop_id=b.shop_id;

drop table if exists online_test_feature_with_knn;
create table online_test_feature_with_knn as select 
	a.*,
...
from online_test_feature_with_multi a
left outer join lq_feature_online_test_nearest1  b1 on a.row_id=b1.row_id and a.shop_id=b1.pre_shop_id
left outer join lq_feature_online_test_nearest2  b2 on a.row_id=b2.row_id and a.shop_id=b2.pre_shop_id
left outer join lq_feature_online_test_nearest3  b3 on a.row_id=b3.row_id and a.shop_id=b3.pre_shop_id
left outer join lq_feature_online_test_nearest5  b5 on a.row_id=b5.row_id and a.shop_id=b5.pre_shop_id
left outer join lq_feature_online_test_nearest7  b7 on a.row_id=b7.row_id and a.shop_id=b7.pre_shop_id
left outer join lq_feature_online_test_nearest10  b10 on a.row_id=b10.row_id and a.shop_id=b10.pre_shop_id
left outer join lq_feature_online_test_nearest15  b15 on a.row_id=b15.row_id and a.shop_id=b15.pre_shop_id
left outer join lq_feature_online_test_nearest20  b20 on a.row_id=b20.row_id and a.shop_id=b20.pre_shop_id
left outer join lq_feature_online_test_nearest30  b30 on a.row_id=b30.row_id and a.shop_id=b30.pre_shop_id
left outer join lq_feature_online_test_nearest50  b50 on a.row_id=b50.row_id and a.shop_id=b50.pre_shop_id;